...
const PRACTICE_NPC_IMAGE_MAP = {
  transport: "./assets/npc1.png",
  hotel: "./assets/npc2.png",
  restaurant: "./assets/npc3.png",
  museum: "./assets/npc4.png"
};
const CARD_POOL = [
...
const learnPracticeGoFlip = document.getElementById("learn-practice-go-flip");
const learnPracticeGoCollection = document.getElementById("learn-practice-go-collection");
const learnPracticeGoFavorites = document.getElementById("learn-practice-go-favorites");
...
const btnFlipGoCollection = document.getElementById("btn-flip-go-collection");
...
const npcPhotoEl = document.getElementById("npc-photo");
...
const LEARN_ASR_LANG = "en-US";
const LEARN_ASR_MAX_RETRIES = 2;
const LEARN_ASR_SILENCE_TIMEOUT_MS = 12000;
let learnAsrState = {
  status: "idle",
  shouldListen: false,
  retries: 0,
...
function renderStep() {
  ...
  if (npcPhotoEl) {
    const imgSrc = PRACTICE_NPC_IMAGE_MAP[currentModuleId];
    const npcSvg = npcWrap ? npcWrap.querySelector(".npc-svg") : null;
    if (imgSrc) {
      npcPhotoEl.alt = `${mod.npcName} 角色形象`;
      npcPhotoEl.classList.remove("hidden");
      npcPhotoEl.onerror = () => {
        npcPhotoEl.classList.add("hidden");
        if (npcSvg) npcSvg.classList.remove("hidden");
      };
      npcPhotoEl.onload = () => {
        if (npcSvg) npcSvg.classList.add("hidden");
      };
      npcPhotoEl.src = imgSrc;
    } else {
      npcPhotoEl.classList.add("hidden");
      npcPhotoEl.removeAttribute("src");
      if (npcSvg) npcSvg.classList.remove("hidden");
    }
  }
...
function initLearningSpeech() {
  ...
  const setLearnAsrStatus = (status, message, isFail) => {
    learnAsrState.status = status;
    if (status === "listening") {
      learnPracticeSpeech.textContent = "聆听中…";
    } else if (status === "processing") {
      learnPracticeSpeech.textContent = "处理中…";
    } else {
      learnPracticeSpeech.textContent = "语音录入";
    }
    if (typeof message === "string" && message) {
      learnPracticeFeedback.textContent = message;
      learnPracticeFeedback.classList.toggle("fail", Boolean(isFail));
    }
  };
  const clearLearnAsrTimeout = () => {
    if (learnAsrState.timeoutId) {
      window.clearTimeout(learnAsrState.timeoutId);
      learnAsrState.timeoutId = 0;
    }
  };
  const scheduleLearnAsrTimeout = () => {
    clearLearnAsrTimeout();
    learnAsrState.timeoutId = window.setTimeout(() => {
      if (!learnAsrState.shouldListen) return;
      console.warn("[ASR] session timeout, forcing restart");
      setLearnAsrStatus("error", "语音识别超时，正在重试…", true);
      try { learnRecognition.stop(); } catch (_) { /* ignore */ }
    }, LEARN_ASR_SILENCE_TIMEOUT_MS);
  };
  const mergeFinalChunk = (chunk) => {
    const next = String(chunk || "").trim();
    if (!next) return;
    const currNorm = normalize(learnAsrState.finalTranscript);
    const nextNorm = normalize(next);
    if (currNorm && nextNorm && currNorm.includes(nextNorm)) return;
    learnAsrState.finalTranscript = `${learnAsrState.finalTranscript} ${next}`.trim();
  };
  const restartLearnRecognition = (reason) => {
    if (!learnAsrState.shouldListen) return;
    if (learnAsrState.retries >= LEARN_ASR_MAX_RETRIES) {
      setLearnAsrStatus("error", "语音识别失败，请点击“语音录入”重试。", true);
      learnAsrState.shouldListen = false;
      clearLearnAsrTimeout();
      return;
    }
    learnAsrState.retries += 1;
    console.warn(`[ASR] retry #${learnAsrState.retries} because ${reason}`);
    window.setTimeout(() => {
      if (!learnAsrState.shouldListen) return;
      learnAsrState.startRequestedAt = performance.now();
      try {
        learnRecognition.start();
      } catch (_) {
        setLearnAsrStatus("error", "语音引擎忙碌，请稍后重试。", true);
      }
    }, 260);
  };
  learnRecognition = new SR();
  learnRecognition.lang = LEARN_ASR_LANG;
  learnRecognition.interimResults = true;
  learnRecognition.continuous = true;
  learnRecognition.maxAlternatives = 1;
  learnRecognition.onstart = () => {
    learnAsrState.sessionStartedAt = performance.now();
    const startupCost = Math.max(0, learnAsrState.sessionStartedAt - learnAsrState.startRequestedAt);
    console.info(`[ASR] listening started in ${startupCost.toFixed(0)}ms`);
    setLearnAsrStatus("listening", "正在识别英文，请持续说话…", false);
    scheduleLearnAsrTimeout();
  };
  learnRecognition.onresult = (e) => {
    ...
    for (let i = e.resultIndex; i < e.results.length; i += 1) {
      ...
      if (result.isFinal) {
        mergeFinalChunk(transcript);
      } else {
        interim += `${transcript} `;
      }
    }
    learnAsrState.interimTranscript = interim.trim();
    learnPracticeInput.value = learnAsrState.finalTranscript || learnAsrState.interimTranscript;
    const livePreview = learnAsrState.interimTranscript ? `（实时）${learnAsrState.interimTranscript}` : "等待更多语音…";
    learnPracticeFeedback.textContent = `识别结果：${learnAsrState.finalTranscript || livePreview}`;
    learnPracticeFeedback.classList.remove("fail");
    setLearnAsrStatus("listening");
  };
  learnRecognition.onerror = (e) => {
    clearLearnAsrTimeout();
    const code = e && e.error ? e.error : "unknown";
    console.warn(`[ASR] error: ${code}`);
    if (code === "not-allowed" || code === "service-not-allowed") {
      learnAsrState.shouldListen = false;
      setLearnAsrStatus("error", "未获得麦克风权限，请先允许浏览器访问麦克风。", true);
      return;
    }
    if (code === "aborted" && !learnAsrState.shouldListen) {
      setLearnAsrStatus("idle");
      return;
    }
    setLearnAsrStatus("error", `识别异常（${code}），正在重试…`, true);
    restartLearnRecognition(code);
  };
  learnRecognition.onend = (e) => {
    clearLearnAsrTimeout();
    const elapsed = learnAsrState.sessionStartedAt ? Math.max(0, performance.now() - learnAsrState.sessionStartedAt) : 0;
    console.info(`[ASR] session end after ${elapsed.toFixed(0)}ms`, e && e.type ? e.type : "unknown");
    if (learnAsrState.shouldListen) {
      restartLearnRecognition("onend");
      return;
    }
    setLearnAsrStatus("idle", learnAsrState.finalTranscript ? `识别结果：${learnAsrState.finalTranscript}` : "语音识别已停止。", false);
  };
}
...
learnPracticeGoFlip.addEventListener("click", () => {
  openLearningView("view-learning-flip");
  updateLearningPointsText();
  renderFlipGrid();
});
learnPracticeGoCollection.addEventListener("click", () => {
  openLearningView("view-learning-collection");
  renderCollection();
});
learnPracticeGoFavorites.addEventListener("click", () => {
  openLearningView("view-learning-favorites");
  renderFavorites();
});
...
learnPracticeSpeech.addEventListener("click", () => {
  ...
  if (!learnAsrState.shouldListen) {
    learnAsrState.shouldListen = true;
    learnAsrState.retries = 0;
    learnAsrState.interimTranscript = "";
    learnAsrState.finalTranscript = "";
    learnAsrState.startRequestedAt = performance.now();
    learnPracticeFeedback.textContent = "正在启动麦克风…";
    learnPracticeFeedback.classList.remove("fail");
    try {
      learnRecognition.start();
    } catch (_) {
      learnAsrState.shouldListen = false;
      learnPracticeFeedback.textContent = "语音引擎忙碌，请稍后重试。";
      learnPracticeFeedback.classList.add("fail");
      learnPracticeSpeech.textContent = "语音录入";
    }
  } else {
...
btnFlipGoCollection.addEventListener("click", () => {
  openLearningView("view-learning-collection");
  renderCollection();
});
